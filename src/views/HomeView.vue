...
const totalSpent = computed(() =>
  (categories.value ?? []).reduce((s, c) => s + spent(c), 0),
)
  ...
  <div class="min-h-screen flex flex-col bg-[#F5EDD8]">
    ...
    <main class="flex-1 overflow-y-auto px-4 py-4">
      ...
      >
        ...
        <div v-if="firstName" class="text-sm font-bold text-white/80 mb-1" style="font-family: 'Baloo 2', cursive">
          E aí, {{ firstName }}! 👋
        </div>
      ...
      >
        ...
        <p class="text-xs font-semibold text-[#C0252A] leading-snug">
          {{ alert.icon }} {{ alert.name }} tá quase no limite! Só sobrou R$ {{ formatBRL(parseFloat(alert.current_balance)) }}, cabra!
        </p>
      </div>
          ...
          <div class="flex justify-between items-center mb-2.5">
            <div class="flex items-center gap-2.5">
              ...
              <div>
                ...
                <div class="text-[11px] text-[#7A6E5F] mt-0.5">R$ {{ formatBRL(spent(cat)) }} gasto</div>
              </div>
            </div>
            <div class="text-right">
              <div
                class="text-[15px] font-bold"
                :style="{ color: pct(cat) >= 90 ? '#C0252A' : '#1A1008', fontFamily: 'Baloo 2, cursive' }"
              >
